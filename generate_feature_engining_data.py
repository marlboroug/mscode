...
month = 10
source_file_name1 = "good_sample10.csv"
source_file_name2 = "bad10.csv"
experiment_type = 'test_feature_engining+month_' + str(month)
year= None
addsum = True
adddiff = True
addsigma = True
...
def generate_data(fault_month_start, fault_month_stop, good_month_start, good_month_stop, good_training_ratio,
    ...
    logging.warning('reading source file...' + source_file_name2)
    bad =  pd.read_csv(source_file_name2,names= header,nrows = nrows_test)
...
#    bad = multiProcPreprocess2(bad.groupby('SerialNumber'),15,bad['SerialNumber'].unique())
# #get top 15   
...
# add feature enginging
    ...
    bad = multiProcPreprocess(bad.groupby('SerialNumber'),topnum,bad['SerialNumber'].unique())
    logging.warning('after multiProcPreprocess good training disks data...' + str(len(bad)))
    
# done
    
    logging.warning('saving training file...')
    good.to_csv("test_" + str(month) + "_month_feature_Good_python_all.csv", index=None)
    logging.warning('saved training file.')
    
    logging.warning('saving testing file...')
    bad.to_csv("test_" +  str(month) + "_month_feature_bad_python_all.csv", index=None)
