...
nrows_test = None
experiment_type = 'get_jump_point_bad_data'
...
def draw_graph(good_data1,good_data2,bad_data,feature_name):
      plt.cla()
      plt.close('all')
      x1=range(1,len(good_data1) + 1)
      x2=range(1,len(good_data2) + 1)
      x3=range(1,len(bad_data) + 1)
      plt.plot(x1,list(good_data1),label='Good line 6_month',marker='o',color='r')
      plt.plot(x2,list(good_data2),label='Good line 12_month',marker='o',color='g',)
      plt.plot(x3,list(bad_data),label='Bad line',marker='o',color='b',)
...
def analyse_result(good_data1,good_data2,bad_data,feature):
      good1 = []
      good2 = []
      bad = []
      ...
      draw_graph(good1,good2,bad,feature)
...
def generate_data(fault_train_month, fault_test_month, good_month_start, good_month_stop, good_training_ratio,fault_disk_take_top, good_disk_take_top, source_file_name):
    #取出两个月的内容
      ...
      bad8 = multiProcPreprocess2(bad8.groupby('SerialNumber'),bad8['SerialNumber'].unique())
      bad9 = multiProcPreprocess2(bad9.groupby('SerialNumber'),bad9['SerialNumber'].unique())
      bad10 = multiProcPreprocess2(bad10.groupby('SerialNumber'),bad10['SerialNumber'].unique())
    
      bad8.to_csv('bad_8.csv', index=None)
      logging.warning('save 8.')
      bad9.to_csv('bad_9.csv', index=None)
      logging.warning('save 9.')
      bad10.to_csv("bad_10.csv", index=None)
      logging.warning('save 10.')
    ...
    # PreciseTimeStamp,SerialNumber,LabelId
      ...
      for feature in features:
          ...
          analyse_result(bad8,bad9,bad10,feature)
