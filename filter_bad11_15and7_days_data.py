...
nrows_test = None
source_file_name1 = 'all_bad11_features.csv' 
filter_time = '2017-11-15'

experiment_type = 'generate_all_bad11_filter15_7'
year = 2016
...
#          header = f.readline().strip('\n').split(',')
      ...
      bad1 = pd.read_csv(source_file_name1, names=header, nrows = nrows_test)
...
#bad get last data
      df_bad_last = multiProcPreprocess2(bad1.groupby('SerialNumber'),1,bad1['SerialNumber'].unique())
# finish bad get last data

      filter_time_value = datetime.datetime.strptime(filter_time, "%Y-%m-%d")
      df_bad_last['PreciseTimeStamp'] = pd.to_datetime(df_bad_last['PreciseTimeStamp'])
      df_bad_last_filter = df_bad_last[df_bad_last["PreciseTimeStamp"] < filter_time_value]
      df_bad_last_disks  = df_bad_last_filter['SerialNumber'].unique()
      bad1 = bad1[bad1['SerialNumber'].isin(df_bad_last_disks)]

      logging.warning('after filter time before:' + filter_time)
      logging.warning('file 1 : ')
      _do_statistics(bad1)
      
      bad1.to_csv('filter_' + filter_time + '_all_bad_11month.csv', index=None)
